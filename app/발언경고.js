...
module.exports = {
	...
	execute(interaction,message) {
		...
		if(message.hasThread){
			interaction.editReply({content : "해당주제의 토론은 이미 시작되었습니다!" }).catch(_=>{});
			return;
		}

		if(channel.isThread()){
			interaction.editReply({content : "여긴 토론을 생성할 수 없는 채널입니다!" }).catch(_=>{});
			return;
		}
		interaction.editReply({content : "토론을 생성합니다...!" }).catch(_=>{});
